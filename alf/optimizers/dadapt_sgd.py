...
class DAdaptSGD(Optimizer):
    ...
    Arguments:
        ...
        fsdp_in_use (bool):
            If you're using sharded parameters, this should be set to True. 
            The optimizer will attempt to auto-detect this, but if you're 
            using an implementation other than PyTorch's builtin version, 
            the auto-detection won't work.
    """
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if group_lr > 0.0:
                    numerator_weighted += dlr * torch.dot(
                        grad.flatten(), s.flatten()).item()
