...
class NormClippingTest(alf.test.TestCase):
    ...
    def test_clip_by_global_norm(self):
        ...
        clipped_tensors, _ = tensor_utils.clip_by_global_norm(tensors,
                                                              clip_norm=1.0,
                                                              in_place=False)
        self.assertTensorNotClose(self._sqr_norm(tensors),
                                  torch.as_tensor(1.0))
        self.assertTensorClose(self._sqr_norm(tensors), sqr_norm)
        self.assertTensorClose(self._sqr_norm(clipped_tensors),
                               torch.as_tensor(1.0))
