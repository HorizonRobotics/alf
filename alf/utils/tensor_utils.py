...
def batched_index_select(input, dim, index):
    ...
    dim using the entries in index. Index is a LongTensor with the same
    batch dimension as the input, and it is used for indexing the input
    ...
    Args:
        ...
        index (LongTensor) – a 1D tensor containing the indices to index the
            ...
            with the shape [B]
    Returns:
        out (Tensor): the tensor generated by indexing the input with index,
            where the shape of out is [B, ..., 1, ...] and
    """
    assert dim < input.ndim and dim > 0, ("invalid dim {} for indexing a "
                                          "tensor of shape {}".format(
                                              dim, input.shape))
    assert index.ndim == 1, "index should be a 1D tensor"
    assert input.shape[0] == index.shape[0], ("size mis-match along the batch "
                                              "dimension, {} v.s. {}".format(
                                                  input.shape[0],
                                                  index.shape[0]))

    singular_shape = [index.shape[0], *[1] * (input.ndim - 1)]
    expanded_shape = [*input.shape]
    expanded_shape[dim] = 1
    index = index.reshape(singular_shape)
    batch_index = index.expand(*expanded_shape)
    out = input.gather(dim, batch_index)
    return out
