...
"""MPC (CEM) algorithm, very similar to TD-MPC"""
...
@alf.configurable
class TdMpcAlgorithm(OffPolicyAlgorithm):
    def __init__(
            self,
            observation_spec,
            action_spec,
            ...
            planner_algorithm_ctor: Callable[..., RLAlgorithm] = MPPIPlanner,
        ...
        planner = planner_algorithm_ctor(
        ...
        super().__init__(
            ...
            action_spec=action_spec,
            reward_spec=reward_spec,
            train_state_spec=planner.train_state_spec,
            predict_state_spec=planner.predict_state_spec,
            rollout_state_spec=planner.rollout_state_spec,
            config=config,
            debug_summaries=debug_summaries,
            name=name)
    ...
    def predict_step(self, time_step: TimeStep, state) -> AlgStep:
        ...
        with torch.cuda.amp.autocast(self._enable_amp):
            ...
            return self._planner.predict_step(
    ...
    def rollout_step(self, time_step: TimeStep, state) -> AlgStep:
        ...
        return self._planner.rollout_step(
            time_step._replace(observation=latent), state)
