...
OaecState = namedtuple(
    "OaecState",
    ['action', 'actor', 'target_actor', 'conservative_actor', 'critics'],
    default_value=())
OaecInfo = namedtuple(
    "OaecInfo", [
        "reward", "reward_noise", "mask", "step_type", "discount", "action",
        "action_distribution", "actor_loss", "cons_actor_loss", "critic",
        "discounted_return"
    ],
    default_value=())
OaecLossInfo = namedtuple(
    'OaecLossInfo', ('actor', 'conservative_actor', 'critic'),
    default_value=())
...
class OaecAlgorithm(OffPolicyAlgorithm):
    ...
    def _predict_action(self,
        ...
        return action_dist, action, OaecActionState(
    ...
    def _critic_train_step(self, inputs: TimeStep, state: OaecCriticState,
        ...
        if self._num_sampled_target_q_actions > 0:
            ...
            if self._std_for_overestimate == 'tot':
                ...
                target_q_bootstrap_diff = target_q_bootstrap - target_q_values[:, :, :
                                                                               1]
                # [n_sampled, T*B]
    ...
    def train_step(self, inputs: TimeStep, state: OaecState,
        ...
        actor_state, actor_loss_info = self._actor_train_step(
            ...
            state=state.actor,
            action=action,
            conservative_training=not self._separate_conservative_actor
            and self._conservative_actor_training)
        ...
        if self._separate_conservative_actor:
            cons_action_dist, cons_actor_state = self._conservative_actor_network(
                inputs.observation,
                state=state.conservative_actor.cons_actor_network)
            cons_action = dist_utils.rsample_action_distribution(
                cons_action_dist)
            cons_critic_states, cons_actor_loss_info = self._actor_train_step(
                inputs=inputs,
                state=state.conservative_actor.critics,
                action=cons_action,
                conservative_training=True)
        ...
        critic_states, critic_info = self._critic_train_step(
            ...
            action_dist=target_action_dist)

        state = OaecState(
            action=action_state,
            actor=actor_state,
            target_actor=target_actor_state,
            conservative_actor=cons_actor_state,
    ...
    def calc_loss(self, info: OaecInfo):
        ...
        if self._num_sampled_target_q_actions > 0:
            ...
            target_q_mean = info.critic.target_q_values[:, :, :1 + self.
                                                        _num_bootstrap_critics,
                                                        ...].mean(
                                                            dim=2
                                                        )  # [T, B, ...]
            if self._std_for_overestimate == 'tot':
                target_q_bootstrap = info.critic.target_q_values[:, :, 1:1 +
                                                                 self.
                                                                 _num_bootstrap_critics,
                                                                 ...]
                target_q_bootstrap_diff = target_q_bootstrap - target_q
                # [T, B, ...]
                target_overest_std = (target_q_bootstrap_diff
                                      **2).mean(dim=2).sqrt()
            else:
                target_q_opt_ptb = info.critic.target_q_values[:, :, -self.
                                                               _num_opt_ptb_critics:,
                                                               ...]
                target_q_opt_ptb_diff = target_q_opt_ptb - target_q
                # [T, B, ...]
                target_overest_std = (target_q_opt_ptb_diff
                                      **2).mean(dim=2).sqrt()
        ...
        if self._use_common_target_q:
            ...
            original_target_value = info.critic.target_q_values[:, :, 1:1 +
                                                                self.
                                                                _num_bootstrap_critics,
                                                                ...].mean(
                                                                    dim=2
        ...
        for i in range(self._num_bootstrap_critics):
            ...
            if self._use_common_target_q:
                ...
                bootstrap_target_value = torch.cat(
                    (target_values_before, target_values_after),
                    dim=2).mean(dim=2)
            if self._reward_noise_scale:
        ...
        if self._debug_summaries and alf.summary.should_record_summaries():
            with alf.summary.scope(self._name):
                ...
                if self._num_sampled_target_q_actions == 0:
                    ...
                    safe_mean_hist_summary("target_overest_std",
                                           target_overest_std)
                safe_mean_hist_summary("opt_ptb_weights",
                                       self._opt_ptb_weights)
