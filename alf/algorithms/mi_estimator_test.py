...
class MIEstimatorTest(parameterized.TestCase, tf.test.TestCase):
    ...
    def test_conditional_mi_estimator(self, estimator='JSD', eps=1000.0,
                                      dim=1):
        ...
        def _get_batch(batch_size, z=None):
            ...
            y_dist = tfp.distributions.Normal(loc=z, scale=tf.ones_like(z))
            y = y_dist.sample()
            x = (z + y * z) * tf.cast(z > 0, tf.float32)
        ...
        @tf.function
        def _train():
            x, y, y_dist = _get_batch(batch_size)
            with tf.GradientTape() as tape:
                alg_step = mi_estimator.train_step((x, y),
        ...
        batch_size = 16384
        x, y, _ = _get_batch(batch_size)
        log_ratio = mi_estimator.calc_pmi(x, y)
        estimated_mi = _calc_estimated_mi(info, log_ratio)
        logging.info("mi=%s estimated_mi=%s" % (float(mi), estimated_mi))
        if estimator == 'JSD':
            self.assertAlmostEqual(estimated_mi, mi, delta=eps)
        else:
            self.assertLess(estimated_mi, mi)
            self.assertGreater(estimated_mi, mi - eps)
        for z in tf.range(-2., 2., 0.125):
            x, y, _ = _get_batch(batch_size, z * tf.ones((batch_size, dim)))
            pmi = mi_estimator.calc_pmi(x, y)
            info = "z=%s mi=%s" % (float(z),
                                   float(0.5 * tf.math.log(1 + z * z) *
                                         tf.cast(z > 0, tf.float32)))
            _calc_estimated_mi(info, pmi)
