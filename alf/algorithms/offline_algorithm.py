...
class OfflineAlgorithm(RLAlgorithm):
    """``OfflineAlgorithm`` implements basic offline RL training pipeline. User
    needs to implement ``train_step()``, which is called to generate necessary information for training.

    The following is the pseudo code to illustrate how ``OfflineAlgorithm``
    is used:
