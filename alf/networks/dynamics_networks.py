...
class DynamicsNetwork(Network):
    ...
    def __init__(self,
        ...
        Args:
            ...
            num_replicas (int): number of replicas to be used in the ensemble
        ...
        if self._prob:
            self._joint_encoder = EncodingNetwork(
                ...
                kernel_initializer=kernel_initializer).make_parallel(
                    num_replicas)
            ...
            self._projection_net = continuous_projection_net_ctor(
                ...
                state_dependent_std=True).make_parallel(num_replicas)
        else:
            self._joint_encoder = EncodingNetwork(
                TensorSpec(
                    (observation_spec.shape[0] + action_spec.shape[0], )),
                ...
                last_layer_size=out_size).make_parallel(num_replicas)
