...
@gin.configurable
class InverseMVPNetwork(Network):
    """Create an instance of InverseMVPNetwork."""
    ...
    def __init__(self,
        ...
        r"""Creates an instance of `InverseMVPNetwork`. 
        When solving an objective of the form :math:`y=J^{-1}*eps` where 
        :math:`J^{-1}` is a square matrix, and :math:`eps` is a vector, solving 
        for this inverse may be numerically unstable or otherwise difficult
        when the dimensionality of :math:`J` is large. This InverseMVP network
        is meant to predict to predict :math:`y` directly. 
        To train InverseMVP, a simple minimization setup is used assuming that
        :math:`J` is available: :math:`||Jy - eps||_2^2`, where the InverseMVP
        network is trained to predict a :math:`y` that minimizes this objective. 
        InverseMVPNetwork is primarily used to train ``svgd3`` when the 
        ``functional_gradient`` argument is set to ``True``. In this case,
        :math:`J^{-1}` is the inverse of the Jacobian of the generator function
        with respect to input :math:`z'`, and :math:`eps` is the gradient of the
        kernel :math:`\nabla_{z'}(z', z)`. 
        The network is designed to take two independent inputs, and concatenate
        them after a linear transformaiton. Following the concatenation of the 
        branches, FC layers are used to process this ``joint"
        representation. 

        Args:
            input_tensor_spec: A TensorSpec representing z_spec or a tuple of 
            TensorSpecs (z_spec, eps_spec).
            output_dim (int): total output dimension of the InverseMVP net,
                will differ between ``svgd3`` and ``minmax`` methods
            hidden_size (int): base hidden width for InverseMVP network
            num_hidden_layers (int): number of hidden layers for InverseMVP net.
            joint_fc_layer_params: (tuple(int)) widths for each layer in the 
                InverseMVP network after concatenating the transformed inputs.
                By default, the width of the joint layers are twice the
        ...
        if joint_fc_layer_params is None:
            joint_fc_layer_params = (joint_hidden_size, ) * num_hidden_layers
