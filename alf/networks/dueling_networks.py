...
from .encoding_networks import Network, EncodingNetwork, LSTMEncodingNetwork, ParallelEncodingNetwork
...
@alf.configurable
class CriticDuelingNetwork(Network):
    ...
    def __init__(self,
        ...
        Args:
            ...
            joint_fc_layer_params (tuple[int]): a tuple of integers representing
                ...
                actions.
            value_vc_layer_params (tuple[int]): a tuple of integers representing
                hidden FC layer sizes for the value branch.
        ...
        adv_net = EncodingNetwork(
            input_tensor_spec=(obs_encoder.output_spec,
                               action_encoder.output_spec),
            output_tensor_spec=output_tensor_spec,
            preprocessing_combiner=observation_action_combiner,
            fc_layer_params=adv_fc_layer_params,
            activation=activation,
            kernel_initializer=kernel_initializer,
    ...
    def forward(self, input, state=()):
        o = self._obs_encoder(input[0])[0]
        a = self._action_encoder(input[1])[0]
        adv = self._adv_net((o, a))[0]
        value = self._value_net(o)[0]
        return (value, adv), state
    ...
    def make_parallel(self, n):
        ...
        """
        return ParallelCriticDuelingNetwork(
            self._obs_encoder, self._action_encoder, self._adv_net,
            self._value_net, n, "parallel_" + self._name)


class ParallelCriticDuelingNetwork(Network):
    def __init__(self, obs_encoder, action_encoder, adv_net, value_net, n,
                 name):
        super().__init__(
            input_tensor_spec=(obs_encoder.input_tensor_spec,
                               action_encoder.input_tensor_spec),
            name=name)
        self._obs_encoder = obs_encoder.make_parallel(n, True)
        self._action_encoder = action_encoder.make_parallel(n, True)
        self._adv_net = adv_net.make_parallel(n)
        self._value_net = value_net.make_parallel(n)
