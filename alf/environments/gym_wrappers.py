...
class Gym3RenderWrapper(gym.Wrapper):
    ...
    def __init__(self, gym_env):
        ...
        """
        import gym3
        assert isinstance(gym_env, gym3.interop.ToGymEnv), \
            f'Gym3RenderWrapper requires gym3 wrapped environment, not {type(gym_env)}'
    ...
    def render(self, mode: str):
        ...
        if not self._render_enabled:
            self.__init__(
                self.env.spec.make(render=(mode == 'human'), render_mode=mode))
            self._render_enabled = True

        # For rgb_array rendering, return the image frame
        wrapped_env = self.env.env
        if len(wrapped_env.get_info()) > 0:
            if 'rgb' in wrapped_env.get_info()[0]:
                return wrapped_env.get_info()[0]['rgb']
        return None
