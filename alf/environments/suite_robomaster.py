...
try:
    ...
    from robomaster import protocol, util, gripper, robotic_arm
except ImportError:
    robomaster = Mock()
...
class RobomasterEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if speed == 0 and turn_speed == 0 and (arm_x != 0 or arm_y != 0):
            ...
            self._robot.robotic_arm.async_move(x=1000 * arm_x, y=1000 * arm_y)
        if gripper_power < 0:
...
class QRCodeRewardFunction(RewardFunction):
    def __init__(self):
        ...
        self.task_space = Box(0, 1, shape=(), dtype=np.int32)
    ...
    def __call__(self, obs):
        ...
        if data == self._words[self._current_word_id]:
            ...
            reward = 0
        return reward, np.int32(self._current_word_id)
